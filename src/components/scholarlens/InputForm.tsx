...
export default function InputForm() {
  ...
  const [corpusPapers, setCorpusPapers] = useState<string[]>([]);

  useEffect(() => {
    fetch("/api/scholarlens")
      .then((res) => res.json())
      .then((data) => {
        if (data.paper_ids) setCorpusPapers(data.paper_ids);
      })
  ...
  async function onSubmit(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/scholarlens", {
        ...
        body: JSON.stringify({ question, paper_ids: corpusPapers }),
      });
  ...
  return (
    <section>
      <form onSubmit={onSubmit} className="flex flex-col gap-3">
        ...
        <button
          type="submit"
          disabled={loading || question.trim().length === 0 || corpusPapers.length === 0}
